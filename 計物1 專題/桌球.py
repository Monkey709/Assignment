...
pp=sphere(pos=(0.38,0.25,0.6),radius=0.02,make_trail=true,color=color.yellow,retain = 600)
...
wall1=box(pos=(3.2,0,0),length=0.01,height=0.4,width=1.525,color=color.white)
...
while True:
    ...
    while t<2 :
        ...
        v = sqrt(xv0**2+yv0**2)
        a0 = f(v)
        F=0.5*den*(av**2)*A*0.45
        g= -9.8
        g+=ii*abs(F/m)
        if z1 <= 0.021 :
            cor = 0.9054*cor                            #for coefficient of restitution = 0.9054
            if cor == 0.9054**4:                       #maximum 4 times pingpong touch table
                #print 3.12 - x1                          #for calculate L
                break
